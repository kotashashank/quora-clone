import React from "react";
import "./WidgetContent.css";

function WidgetContent() {
  return (
    <div className="widget__contents">
      <div className="widget__content">
        <img
          src="https://pyxis.nymag.com/v1/imgs/160/7b8/be9b71674a92129013428120b1a0c699b7-howtogetintodrawing.rsquare.w700.jpg"
          alt=""
        />
        <div className="widget__contentTitle">
          <h5>[Self] iPhone art</h5>
          <p>I recently dropped my ...</p>
        </div>
      </div>
      <div className="widget__content">
        <img
          src="https://qphs.fs.quoracdn.net/main-thumb-ti-1644613-100-ydflucgoeztbhwyurtmlqqrgfqmjmhpl.jpeg"
          alt=""
        />
        <div className="widget__contentTitle">
          <h5>The Art vs The Artist</h5>
          <p>After a fair share of ...</p>
        </div>
      </div>
      <div className="widget__content">
        <img
          src="https://qphs.fs.quoracdn.net/main-thumb-t-801-100-Sf8h894FXbQZQit0TeqDrrqS6xw6dwCQ.jpeg"
          alt=""
        />
        <div className="widget__contentTitle">
          <h5>Anchor out now</h5>
          <p>I finished my newest ...</p>
        </div>
      </div>
      <div className="widget__content">
        <img
          src="https://www.thesprucecrafts.com/thmb/eZc0kzin-wIqynkBVzsXs0qZXsY=/3064x3064/smart/filters:no_upscale()/close-up-of-man-photographing-through-dslr-736206565-59a8bd2b054ad90010f15275.jpg"
          alt=""
        />
        <div className="widget__contentTitle">
          <h5>Nothing like Ontario</h5>
          <p>Went for a evening run ...</p>
        </div>
      </div>
      <div className="widget__content">
        <img
          src="https://pyxis.nymag.com/v1/imgs/160/7b8/be9b71674a92129013428120b1a0c699b7-howtogetintodrawing.rsquare.w700.jpg"
          alt=""
        />
        <div className="widget__contentTitle">
          <h5>F500 Art</h5>
          <p>Made some hills based ...</p>
        </div>
      </div>
      <div className="widget__content">
        <img
          src="https://ae01.alicdn.com/kf/Hf6c82e56a241413d855d8d59053e78e5e/Pottery-Tools-Cutting-Knife-Sculpture-Carving-Craft-Supplies-Clay-Cutters-Ceramic-DIY-Design-Sculpting-Trimming-Modelling.jpg_Q90.jpg_.webp"
          alt=""
        />
        <div className="widget__contentTitle">
          <h5>Arch. Project: Phase 3</h5>
          <p>Just finished the latest...</p>
        </div>
      </div>
      <div className="widget__content">
        <img
          src="https://qphs.fs.quoracdn.net/main-thumb-t-801-100-Sf8h894FXbQZQit0TeqDrrqS6xw6dwCQ.jpeg"
          alt=""
        />
        <div className="widget__contentTitle">
          <h5>Second Verse!!!</h5>
          <p>The second verse now ...</p>
        </div>
      </div>
      <div className="widget__content">
        <img
          src="https://qphs.fs.quoracdn.net/main-thumb-ti-1644613-100-ydflucgoeztbhwyurtmlqqrgfqmjmhpl.jpeg"
          alt=""
        />
        <div className="widget__contentTitle">
          <h5>Finally figured out ... </h5>
          <p>After hours, drawing ...</p>
        </div>
      </div>
    </div>
  );
}

export default WidgetContent;
